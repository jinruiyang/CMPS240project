...
# This is a Neural Network model with pre-trained word embeddings.
# Here is a list of resources I found very helpful in building this model:
#
# For beggining and building the sequential model I found the tensorflow
# website very helpful, especially a tutorial on basic text classification:
# https://www.tensorflow.org/tutorials/keras/basic_text_classification
#
# For help with downloading pre-trained word embeddings and creating the 
# embedding matrices, I found a Kaggle kernel very helpful:
# https://www.kaggle.com/sudalairajkumar/a-look-at-different-embeddings
#
#
# This model is meant to be run as a Kaggle Kernel because it takes advantage of 
# the built in GRU.
#
...
# Missspelled dictionary with common mispelled words and expanded out contractions
...
# fucntion that cleans the text with preprocessing steps
def clean_text(text):
    # clean latex maths
...
max_len = 70                # length of every instance
...
# create an embedding matrix with pre-trained word vectors from wiki-news
...
# create an embedding matrix with pre-trained word vectors from glove
...
for word, i in word_index.items():
   ...
   # create an embedding matrix with pre-trained word vectors from paragram
...
# Params: Embedding matrix, can be NoneType
# input is secquences of length max_len
# Embedding layer with specified embeddings matrix
# Bidirectional GRU layer
# concatenate and average pooling layer and a max pool layer
# dense relu activation
# .1 dropout
# dense sigmoid activation as final output
...
# create a model with wiki-news embeddings
...
# test F1 score at different threshholds for the validation set
...
# create the predictions for the test set using the trained model. 
# delete model when done with it
wiki_pred_test_y = model.predict([test_seq_x], batch_size=1024, verbose=1)
del model

# create model with paragram embeddings matrix
model = create_model(paragram_embedding_matrix)
print(model.summary())
# fit the model on the training set
model.fit(train_x, train_y, batch_size=512, epochs=2, validation_data=(valid_x, valid_y))
# test f1 score on different thresholds for the validation set
...
# fit the model on the training set
model.fit(train_x, train_y, batch_size=512, epochs=2, validation_data=(valid_x, valid_y))
# test f1 score at different thresholds on the validation set
...
print("Val F1 Score: {:.6f}".format(best_score))
# create predictipns for test set
# delete model
glove_pred_test_y = model.predict([test_seq_x], batch_size=1024, verbose=1)
del model


# evenly weight each models predicitons 
w_coe = .33
p_coe = .33
g_coe = .34
# get a final prediction on the validation set with a mix of scores from different models
...
# defetrmin best threshold for testing
print("Best threshold: " + str(best_thresh))
print("Val F1 Score: {:.6f}".format(best_score))

# create final predictions by weighing each midel equally
# create output file with predictions 'submission.csv'
